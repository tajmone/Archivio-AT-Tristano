Version 1 of Italian by Massimo Stella begins here.
...
[This part of code comes from Emily Short's Punctuation Removal. We'll use just the part we need, for size concerns.]
...
Table of library messages (continued)
Message Id				Message Text
LibMsg <confirm Restart>			"Sei sicuro di voler ricominciare?(y/n)"
...
LibMsg <score changed>		"Il tuo punteggio è [if numeric amount >= 0]incrementato di [the numeric amount in words][otherwise]diminuito [0 minus the numeric amount in words][end if] punti."
...
LibMsg <cannot exit thing not within>			"Ma non puoi [if the main object is a supporter]scendere da[otherwise]uscire da[end if] [=> %][on|in] [the % dobj]."
...
LibMsg <cannot travel in something not a vehicle>	"Dovrai prima [=> %][off|out of] [the % dobj]."
...
LibMsg <say npc arrives>					"[The actor] arriva [from the main object]"
LibMsg <say npc arrives from unknown direction>		"[The actor] arriva."
LibMsg <say npc arrives at>				"[The actor] arriva [the %] [from the secondary object]"
LibMsg <say npc goes through>				"[The actor] va attraverso [the %]"
LibMsg <say npc arrives from>				"[The actor] arriva da [the %]"
...
Instead of examining a closed container (called the item): 
	make sure main object is set / "% dobj";
     	say "[The item] attualmente è chius[genderity]."
...
Definition : something is empty if it carries nothing.

After printing the name of a lit thing (called the item) while taking inventory: 
       say " (che al momento è acces[genderity])"; 
        omit contents in listing. 
After printing the name of an open container (called the item) while taking inventory: 
        if the item is singular-named, say " (apert[genderity])"; 
        omit contents in listing. 
After printing the name of a closed container (called the item) while taking inventory: 
        say " (chius[genderity])"; 
        omit contents in listing. 
After printing the name of an empty container (called the item) while taking inventory: 
         say " (apparentemente vuot[genderity])"; 
         omit contents in listing.
After printing the name of an empty open container (called the item) while printing the locale description: 
         say " (in cui non noti niente di che)";
         omit contents in listing.
...
[ LanguageContraction text;
  if (text->0 == 'a' or 'e' or 'i' or 'o' or 'u' or 'A' or 'E' or 'I' or 'O' or 'U') return 2;
  if (text->0 == 'z' or 'Z' or 'x' or 'X') return 1;
  if (text->0 ~= 's' or 'S') 
	if (text->1 == 'a' or 'e' or 'i' or 'o' or 'u' or 'A' or 'E' or 'I' or 'O' or 'U') return 0;
	else return 1;
  if (text->1 == 'a' or 'e' or 'i' or 'o' or 'u' or 'A' or 'E' or 'I' or 'O' or 'U') return 1;
  return 0;
];
...
Array LanguageArticles -->
 ...
 ! Cdef   Def    Indef   Cdef   Def    Indef   Cdef   Def    Indef
   ...
   "La "  "la "  "una "  "La "  "la "  "una "  "L'"   "l'"   "un'"
   "I "   "i "   "un "   "Gli " "gli " "uno "  "Gli " "gli " "un "
   "Le "  "le "  "una "  "Le " "le " "una "  "Le "  "le "  "un'"; 
                   !             a           i
                   !             s     p     s     p
...
Understand "attacca [something]" or "attacca il/la/i/le/gli/quei/quella/quello/quelle [something]" as attacking.
Understand "uccidi [something]" or "uccidi il/la/i/le/gli/quei/quella/quello/quelle [something]" as attacking.
Understand "prendi [something]" or "prendi il/la/i/le/gli/quei/quella/quello/quelle [something]" as taking.
Understand "mangia [something]" or "mangia il/la/i/le/gli/quei/quella/quello/quelle [something]" as eating.
Understand "bacia [something]" or "bacia il/la/i/le/gli/quei/quella/quello/quelle [something]" as kissing.
Understand "canta" as singing.
Understand "tocca [something]" or "tocca il/la/i/le/gli/quei/quella/quello/quelle [something]" as touching.
Understand "brucia [something]" or "brucia il/la/i/le/gli/quei/quella/quello/quelle [something]" as burning.
Understand "odora" as smelling.
Understand "bevi [something]" or "bevi il/la/i/le/gli/quei/quella/quello/quelle [something]" as drinking.
Understand "compra [something]" or "compra il/la/i/le/gli/quei/quella/quello/quelle [something]" as buying.
Understand "dormi" as sleeping.
Understand "salta" as jumping.
Understand "svegliati" as waking up.
Understand "taglia [something]" or "taglia il/la/i/le/gli/quei/quella/quello/quelle [something]" as burning.
Understand "agita le mani" as waving hands.
Understand "colpisci il/la/i/le/gli/quei/quella/quello/quelle [something]" or "colpisci [something]" as swinging.
Understand "spremi [someone]" or "spremi il/la/i/le/gli/quei/quella/quello/quelle [something]" as squeezing.
Understand "ascolta" as listening.
Understand "verifica" as verifying the story file.
Understand "pensa" or "ricorda" as thinking.
Understand "guarda [something]" or "guarda il/la/i/le/gli/quei/quella/quello/quelle [something]" as examining.
Understand "guarda dentro [something]" or "guarda dentro il/la/i/le/gli/quei/quella/quello/quelle [something]" or "guarda nella/negli/nelle [something]" or "guarda in [something]" as examining.
Understand "esamina [something]" or "esamina il/la/i/le/gli/quei/quella/quello/quelle [something]" as examining.
Understand "calcia [something]" or "calcia il/la/i/le/gli/quei/quella/quello/quelle [something]" as attacking.
Understand "accendi [something]" or "accendi il/la/i/le/gli/quei/quella/quello/quelle [something]" as switching on.
Understand "spegni [something]" or "spegni il/la/i/le/gli/quei/quella/quello/quelle [something]" as switching off.
Understand "spingi [something]" or "spingi il/la/i/le/gli/quei/quella/quello/quelle [something]" as pushing.
Understand "indossa [something]" or "indossa il/la/i/le/gli/quei/quella/quello/quelle [something]" as wearing.
Understand "lascia [something]" or "lascia il/la/i/le/gli/quei/quella/quello/quelle [something]" as dropping.
Talking to is an action applying to one visible thing.
Understand "parla [something]" or "parla col/colla/colle [something]" or "parla al/alla/allo/all/all'/a [something]" or "parla con il/la/i/le/gli/quei/quella/quello/quelle [something]" as talking to.
Understand "apri [something]" or "apri il/la/i/le/gli/quei/quella/quello/quelle [something]" as opening.
Understand "chiudi [something]" or "chiudi il/la/i/le/gli/quei/quella/quello/quelle [something]" as closing.
Understand "sblocca [something] con [something]" or "sblocca il/la/i/le/gli/quei/quella/quello/quelle [something] con il/la/i/le/gli/quei/quella/quello/quelle[something]" as unlocking it with.
Understand "apri [something] con [something]" or "apri il/la/i/le/gli/quei/quella/quello/quelle [something] con il/la/i/le/gli/quei/quella/quello/quelle[something]" as unlocking it with.
Understand "chiudi [something] con [something]" or "chiudi il/la/i/le/gli/quei/quella/quello/quelle [something] con il/la/i/le/gli/quei/quella/quello/quelle[something]" as locking it with.
Understand "mostra [something] a [something]" or "mostra il/la/i/le/gli/quei/quella/quello/quelle [something] al/alla/allo/all/all'/a [something]" as showing it to.
Understand "chiedi a [something] di [text]" or "chiedi al/alla/allo/all/all'/a [something] di/del/dei/dello/della/delle/dei/riguardo/circa [text]" as asking it about.
Understand "rimuovi [something] da [something]" or "rimuovi il/la/i/le/gli/quei/quella/quello/quelle [something] da/dalle/dalla/dagli/dai [something]" as removing it from.
Understand "poggia [something] su [something]" or "poggia il/la/i/le/gli/quei/quella/quello/quelle [something] sul/sullo/sulla/sulle/sugli/sopra il/la/i/le/gli// [something]" as putting it on.
Understand "inserisci [something] dentro [something]" or "inserisci il/la/i/le/gli/quei/quella/quello/quelle [something] nel/nello/negli/nelle [something]" as inserting it into.
Understand "parla a/al/alla/alle/agli/ai/allo [something] riguardo/di/del/della/dello/delle/degli [text]" as telling it about.
Understand "interroga il [something] riguardo [text]" as telling it about.
Understand "chiedi a/al/alla/alle/agli/ai/allo [something] riguardo/di/del/della/dello/delle/degli [text]" as telling it about.
Understand "vai [something]" or "vai a [something]" as going.
Understand "scendi dalla [something]" as getting off.
Understand "sali sul/sullo/sulla/sulle/sugli/sui [something]" as entering.
Understand "entra nello/nella/in/nel/nei/negli/nelle [something]" as entering.
...
Example: * Atto 0 - Una avventura thriller per Inform 7 di Massimo Stella.
	...
	*: "Atto 0" by Massimo Stella
	...
	Rilievo is a kind of value. The rilievos are prendendo alcune impronte, scattando alcune foto dei particolari del cadavere, osservando la forma di alcuni schizzi di sangue, guardando il proprio orologio, prelevando un campione di sangue, osservando meglio la 	scena del crimine, rivedendo le foto scattate.
